...
int addSymbol(Symbol** table, char* name, int scopeLevel, DataType type)
{
   ...
   int index = hash(name);
   Symbol* cursor = (Symbol*)malloc(sizeof(Symbol));
   cursor->name = strdup(name);
   cursor->scopeLevel = scopeLevel;
...
Symbol* findSymbol(Symbol** table, char* name)
{
   ...
   int index = hash(name);
   Symbol *cur = table[index];
   //check if table[index] = null and return null if null
   if (table[index]==0){
	   return NULL;
   }
   // else check if position of table at that index = name passed from method, if not, then iterate through list
   // while loop where you assign a new symbol, cur = table[index if cur-> name = name return cur, else cur = cur.next while cur is not null
   else{
	while(cur!=NULL){
